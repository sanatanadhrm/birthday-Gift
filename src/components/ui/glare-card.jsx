...
export const GlareCard = ({ idx, product, className }) => {
  ...
  useEffect(() => {
    updateStyles(); // Panggil update saat komponen dipasang
  }, []);
  ...
  return (
    <div
      ...
      className="item-card relative [contain:layout_style] [perspective:600px] transition-transform duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] will-change-transform w-[320px] [aspect-ratio:17/21] cursor-pointer"
      ref={refElement}
      onPointerMove={(event) => {
        ...
        <div className="w-full h-full grid [grid-area:1/1] mix-blend-soft-light [clip-path:inset(0_0_0_0_round_var(--radius))]">
          <div className={cn("h-full w-full bg-slate-950", className)}>
            <div>
              ...
              <h2 className="absolute bottom-2 left-2 sm:bottom-4 sm:left-4 opacity-0 group-hover/product:opacity-100 text-white text-sm sm:text-lg">
                {product.title}
              </h2>
            </div>
          </div>
